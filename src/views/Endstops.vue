					...
					<tr v-for="i in [0, 1, 2]">
						...
						<td>
							<b-form-radio-group buttons button-variant="outline-primary" :checked="template.drives[i].endstop_type" @change="updateDrive({ drive: i, et: $event })" v-preset.left="preset.drives[i].endstop_type" :title="$t('endstops.endstopTypeDescription')" :name="'endstopType' + i" class="w-100">
								...
								<b-form-radio :value="4" class="w-100" :disabled="!board.hasMotorLoadDetection || (template.drives[i].stepperDriver !== 'TMC2209' && template.drives[i].stepperDriver !== 'TMC2226' && template.drives[i].stepperDriver !== 'TMC5160') " v-b-tooltip.bottom title="Motor stall detection of the stepper drivers">Sensorless</b-form-radio>
							</b-form-radio-group>
						</td>
						<td>
